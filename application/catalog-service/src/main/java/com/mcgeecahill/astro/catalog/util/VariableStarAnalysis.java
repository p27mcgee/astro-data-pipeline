 ...
 *
 * <p>This implementation incorporates time-series analysis methodologies from Space Telescope
 * Science Institute (STScI) variable star research and the Lightkurve project for time-series
 * photometry analysis.
 *
 * <p>STScI CODE REFERENCES AND COMPATIBILITY:
 *
 * <p>1. Lightkurve Time-Series Analysis: - Reference: https://github.com/spacetelescope/lightkurve
 * - STScI Implementation: Time-series analysis for Kepler, TESS, K2 data - License: MIT License
 * (compatible with BSD) - Compatibility: ✅ FULL COMPATIBILITY - MIT and BSD are compatible - Our
 * implementation: Independent Java implementation of time-series algorithms
 *
 * <p>2. STScI Variable Star Research: - Reference: STScI variable star catalogs and period analysis
 * - STScI Implementation: Professional variable star classification - License: Scientific
 * publications (public domain) - Compatibility: ✅ FULL COMPATIBILITY - Public domain algorithms -
 * Our implementation: Java adaptation of published algorithms
 *
 * <p>3. Periodogram Analysis (Lomb-Scargle): - Reference: Classical astronomical period detection -
 * Implementation: Standard Lomb-Scargle periodogram - License: Public domain (classical algorithm)
 * - Compatibility: ✅ FULL COMPATIBILITY - Public domain - Our implementation: Java implementation
 * of Lomb-Scargle method
 *
 * <p>License compatibility: MIT + BSD = fully compatible for all uses
 */
import static com.mcgeecahill.astro.catalog.constants.AstronomicalConstants.HOURS_PER_DAY;
...
public class VariableStarAnalysis {
     ...
     *
     * <p>This implementation follows the time-series analysis approach used in the STScI Lightkurve
     * project for variable star detection and classification.
     *
     * <p>Reference: STScI Lightkurve time-series analysis
     * https://github.com/spacetelescope/lightkurve
     *
     * <p>License compatibility: MIT License (compatible with our BSD-style license)
     *
     ...
     */
    public VariabilityResult analyzeVariability(
            double[] times, double[] magnitudes, double[] errors) {
        log.debug(
                "STScI-style variability analysis: {} observations spanning {:.2f} days",
                times.length,
        ...
        if (times.length < MIN_OBSERVATIONS) {
            ...
            return new VariabilityResult(
                    false, 0.0, 0.0, 0.0, VariabilityType.CONSTANT, 0.0, 0.0, new HashMap<>());
        }
        ...
        Map<String, Double> additionalMetrics =
                calculateAdditionalMetrics(times, magnitudes, periodogram.bestPeriod);

        boolean isVariable =
                stats.variabilityIndex > QualityAssessmentConstants.VARIABILITY_INDEX_THRESHOLD
                        && periodogram.bestPower
                                > QualityAssessmentConstants.PERIODOGRAM_POWER_THRESHOLD;

        log.debug(
                "Variability analysis complete: variable={}, period={:.6f}d, type={}",
                isVariable,
    ...
    /**
     * Calculate basic variability statistics. Uses STScI methodology for variability assessment.
     */
    private VariabilityStatistics calculateVariabilityStatistics(
            double[] magnitudes, double[] errors) {
        double sum = 0.0;
     ...
     *
     * <p>This implements the classical Lomb-Scargle algorithm widely used in astronomical
     * time-series analysis, following STScI Lightkurve methodology.
     *
     * <p>Reference: Lomb-Scargle periodogram (classical algorithm) STScI Implementation: Used in
     * Lightkurve for period detection License compatibility: Public domain algorithm
     */
    private PeriodogramResult calculateLombScarglePeriodogram(
            double[] times, double[] magnitudes, double[] errors) {
        ...
        double maxFreq = 1.0 / (MIN_PERIOD_HOURS / HOURS_PER_DAY);
        ...
        for (int f = 0; f < nFreq; f++) {
            ...
            double cosTerm =
                    (sumCosNum * sumCosNum)
                            / Math.max(
                                    sumCosDen,
                                    QualityAssessmentConstants.LOMB_SCARGLE_MIN_DENOMINATOR);
            double sinTerm =
                    (sumSinNum * sumSinNum)
                            / Math.max(
                                    sumSinDen,
            ...
            powers[f] = QualityAssessmentConstants.LOMB_SCARGLE_POWER_FACTOR * (cosTerm + sinTerm);
        ...
        log.debug(
                "Lomb-Scargle periodogram: best period={:.6f}d, power={:.2f}",
                bestPeriod,
                bestPower);
    ...
    /**
     * Classify variable star type based on statistical properties. Uses STScI variable star
     * classification methodology.
     */
    private VariabilityType classifyVariableType(
            VariabilityStatistics stats, PeriodogramResult periodogram) {

        if (stats.variabilityIndex < QualityAssessmentConstants.VARIABILITY_INDEX_THRESHOLD) {
        ...
        if (period < QualityAssessmentConstants.RR_LYRAE_PERIOD_THRESHOLD
                && amplitude > QualityAssessmentConstants.RR_LYRAE_AMPLITUDE_THRESHOLD) {
            return VariabilityType.RR_LYRAE;
        } else if (period >= QualityAssessmentConstants.CEPHEID_PERIOD_MIN
                && period < QualityAssessmentConstants.CEPHEID_PERIOD_MAX
                && amplitude > QualityAssessmentConstants.CEPHEID_AMPLITUDE_THRESHOLD) {
            return VariabilityType.CEPHEID;
        } else if (period > QualityAssessmentConstants.LONG_PERIOD_MIN
                && period < QualityAssessmentConstants.LONG_PERIOD_MAX) {
            return VariabilityType.LONG_PERIOD_VARIABLE;
        } else if (amplitude < QualityAssessmentConstants.MICRO_VARIABLE_AMPLITUDE_THRESHOLD) {
            return VariabilityType.MICRO_VARIABLE;
        } else if (periodogram.bestPower < QualityAssessmentConstants.IRREGULAR_POWER_THRESHOLD) {
    ...
    /**
     * Calculate additional time-series metrics. Includes metrics used in STScI variable star
     * research.
     */
    private Map<String, Double> calculateAdditionalMetrics(
            double[] times, double[] magnitudes, double bestPeriod) {
    ...
    private double calculatePhaseFoldedScatter(double[] phases, double[] magnitudes) {
        ...
        int nBins = QualityAssessmentConstants.PHASE_BINS;
    ...
    private double calculateSkewness(double[] magnitudes) {
        double mean = Arrays.stream(magnitudes).average().orElse(0.0);
        double variance =
                Arrays.stream(magnitudes).map(m -> (m - mean) * (m - mean)).average().orElse(1.0);
        double stdDev = Math.sqrt(variance);

        double skewness =
                Arrays.stream(magnitudes)
    ...
    /** Calculate Stetson J variability index. Standard astronomical variability measure. */
    ...
    private static class VariabilityStatistics {
        ...
        VariabilityStatistics(
                double meanMagnitude, double rms, double amplitude, double variabilityIndex) {
    ...
    public record VariabilityResult(
            boolean isVariable,
            double period,
            double periodPower,
            double amplitude,
            VariabilityType variableType,
