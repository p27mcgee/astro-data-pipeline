...
import static com.mcgeecahill.astro.catalog.constants.AstronomicalConstants.ARCSEC_TO_DEG;
import static com.mcgeecahill.astro.catalog.constants.QualityAssessmentConstants.DEFAULT_MATCH_RADIUS_ARCSEC;
...
public class CatalogService {
    ...
    private static final double DEGREES_TO_METERS =
    ...
    public ConeSearchResult performConeSearch(ConeSearchRequest request) {
        log.debug(
                "Performing cone search: RA={}, Dec={}, radius={}\"",
                request.getCenterRa(),
                request.getCenterDec(),
                request.getRadiusArcsec());

        // Convert arcseconds to meters for PostGIS query
        double radiusMeters = request.getRadiusArcsec() * ARCSEC_TO_DEG * DEGREES_TO_METERS;

        List<AstronomicalObject> objects;
        if (request.getPageable() != null) {
            Page<AstronomicalObject> page =
                    objectRepository.findWithinRadius(
                            request.getCenterRa(),
                            request.getCenterDec(),
                            radiusMeters,
                            request.getPageable());
            ...
            objects =
                    objectRepository.findWithinRadius(
                            request.getCenterRa(), request.getCenterDec(), radiusMeters);
        }

        // Calculate separations and apply additional filters
        List<ConeSearchResult.ObjectMatch> matches =
                objects.stream()
                        .map(
                                obj -> {
                                    double separation =
                                            astroCalculations.calculateSeparation(
                        ...
                        .filter(
                                match ->
                                        request.getObjectTypes() == null
                                                || request.getObjectTypes()
                                                        .contains(
                                                                match.getObject()
                                                                        .getObjectType()
                                                                        .name()))
                        .filter(
                                match ->
                                        request.getMinMagnitude() == null
                                                || (match.getObject().getMagnitude() != null
                                                        && match.getObject().getMagnitude()
                                                                >= request.getMinMagnitude()))
                        .filter(
                                match ->
                                        request.getMaxMagnitude() == null
                                                || (match.getObject().getMagnitude() != null
                                                        && match.getObject().getMagnitude()
                                                                <= request.getMaxMagnitude()))
                        .sorted(
                                Comparator.comparing(
                                        ConeSearchResult.ObjectMatch::getSeparationArcsec))
                        .limit(request.getMaxResults() != null ? request.getMaxResults() : 1000)
                        .collect(Collectors.toList());

        // Extract objects from matches for backward compatibility
        List<AstronomicalObject> resultObjects =
                matches.stream()
    ...
    public Optional<ObjectMatchResult> findNearestObject(
            double ra,
            double dec,
            AstronomicalObject.ObjectType objectType,
            double maxSeparationArcsec) {
        log.debug(
                "Finding nearest {} object to RA={}, Dec={} within {}\"",
                objectType,
                ra,
                dec,
                maxSeparationArcsec);

        double radiusMeters = maxSeparationArcsec * ARCSEC_TO_DEG * DEGREES_TO_METERS;

        List<Object[]> results =
                objectRepository.findNearestObjectsOfType(
        ...
        return Optional.of(
                ObjectMatchResult.builder()
                        .object(object)
                        .separationArcsec(separationArcsec)
                        .matchConfidence(
                                calculateMatchConfidence(separationArcsec, maxSeparationArcsec))
                        .build());
    }
                ...
                .map(
                        pos ->
                                findNearestObject(
                                        ...
                                        pos.getRadiusArcsec() != null
                                                ? pos.getRadiusArcsec()
                                                : DEFAULT_MATCH_RADIUS_ARCSEC))
        ...
        for (int i = 0; i < objects.size(); i += batchSize) {
            ...
            for (AstronomicalObject object : batch) {
                try {
                    ...
                    log.warn(
                            "Failed to import object {}: {}", object.getObjectId(), e.getMessage());
                }
            }
    ...
    public Map<String, Object> getCatalogStatistics() {
        ...
        List<AstronomicalObject> recentObservations =
                objectRepository.findRecentlyObserved(lastWeek);
    ...
    public Page<AstronomicalObject> getObjectsInRegion(
        ...
        return objectRepository.findByTypeAndMagnitudeInRegion(
                objectType, minMagnitude, maxMagnitude, minRa, maxRa, minDec, maxDec, pageable);
    }
