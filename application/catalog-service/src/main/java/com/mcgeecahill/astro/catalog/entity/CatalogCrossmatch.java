...
import static com.mcgeecahill.astro.catalog.constants.AstronomicalConstants.ARCSECONDS_PER_ARCMINUTE;
import static com.mcgeecahill.astro.catalog.constants.QualityAssessmentConstants.DEFAULT_MATCH_RADIUS_ARCSEC;
import static com.mcgeecahill.astro.catalog.constants.QualityAssessmentConstants.HIGH_MATCH_CONFIDENCE_THRESHHOLD;
...
public class CatalogCrossmatch {
    ...
    public boolean isHighConfidenceMatch() {
        return matchConfidence != null
                && matchConfidence > HIGH_MATCH_CONFIDENCE_THRESHHOLD
                && separationArcsec != null
                && separationArcsec < DEFAULT_MATCH_RADIUS_ARCSEC;
    }

    /** Get formatted separation */
    public String getFormattedSeparation() {
        if (separationArcsec != null) {
            if (separationArcsec < DEFAULT_MATCH_RADIUS_ARCSEC) {
                return String.format("%.2f\"", separationArcsec);
            } else if (separationArcsec < ARCSECONDS_PER_ARCMINUTE) {
                return String.format("%.1f\"", separationArcsec);
            } else {
                return String.format("%.1f'", separationArcsec / ARCSECONDS_PER_ARCMINUTE);
            }
        }
        return "N/A";
    }
