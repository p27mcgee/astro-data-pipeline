package com.mcgeecahill.astro.catalog.repository;
...
public interface AstronomicalObjectRepository extends JpaRepository<AstronomicalObject, Long> {
    ...
    List<AstronomicalObject> findByObjectTypeAndMagnitudeIsNotNull(
    ...
    @Query(
            "SELECT ao FROM AstronomicalObject ao "
    ...
    List<AstronomicalObject> findByMagnitudeRange(
            @Param("minMag") double minMag, @Param("maxMag") double maxMag);
    ...
    // Cross-match queries
    @Query("SELECT ao FROM AstronomicalObject ao " + "WHERE ao.gaiaSourceId = :gaiaId")
    Optional<AstronomicalObject> findByGaiaSourceId(@Param("gaiaId") Long gaiaId);

    @Query("SELECT ao FROM AstronomicalObject ao " + "WHERE ao.simbadName = :simbadName")
    ...
    List<AstronomicalObject> findVariableStarsInPeriodRange(
            @Param("minPeriod") double minPeriod, @Param("maxPeriod") double maxPeriod);
    ...
    @Query(
            "SELECT AVG(ao.magnitude) FROM AstronomicalObject ao "
    ...
    @Query(
            "SELECT MIN(ao.magnitude), MAX(ao.magnitude) FROM AstronomicalObject ao "
                    + "WHERE ao.objectType = :objectType AND ao.magnitude IS NOT NULL")
    ...
    List<AstronomicalObject> findHighProperMotionObjects(
            @Param("minProperMotion") double minProperMotion);
    ...
    @Query(
            value =
                    """
                SELECT ao.*,
                                   ST_Distance(ao.position::geography,
                                   ST_Point(:targetRa, :targetDec)::geography)
                                    / 1000.0 * 206265 as separation_arcsec
    ...
    void deleteByObjectTypeAndLastObservedBefore(
            AstronomicalObject.ObjectType objectType, LocalDateTime cutoffDate);
}
