...
def discover_batch_files(**context) -> Dict[str, Any]:
    ...
    logging.info(
        f"Batch processing configuration: start_date={start_date}, "
    ...
    try:
        ...
        page_iterator = paginator.paginate(Bucket=S3_BUCKET_RAW, Prefix=s3_prefix)
        ...
        logging.info(
            f"Found {len(batch_files)} files for batch processing " f"(total size: {total_size / (1024**3):.2f} GB)"
        )
...
def process_batch_parallel(**context) -> Dict[str, Any]:
        ...
        for file_info in files:
            try:
                ...
                if response.status_code == 202:
                    ...
                    processed_files.append({"file_info": file_info, "job_id": job_data["jobId"], "status": "SUBMITTED"})
                else:
                    failed_files.append({"file_info": file_info, "error": f"HTTP {response.status_code}"})
    ...
    with ThreadPoolExecutor(max_workers=MAX_PARALLEL_JOBS) as executor:
        future_to_batch = {executor.submit(process_single_batch, batch): batch for batch in batch_jobs}
        ...
        for future in as_completed(future_to_batch):
            ...
            try:
                ...
                logging.info(
                    f"Completed batch {result['batch_id']}: "
...
def monitor_batch_completion(**context) -> Dict[str, Any]:
    ...
    while time.time() - start_time < max_wait_time:
        ...
        logging.info(f"Job status: {completed_jobs} completed, {failed_jobs} failed, " f"{len(pending_jobs)} pending")
        time.sleep(check_interval)

    result = {"completed_jobs": completed_jobs, "failed_jobs": failed_jobs, "pending_jobs": len(all_job_ids)}

    logging.info(
        f"Monitoring complete: {completed_jobs} completed, " f"{failed_jobs} failed, {len(all_job_ids)} still pending"
    )
...
all_tasks = [discover_task, create_jobs_task, processing_group, cleanup_task, validation_task, report_task]
