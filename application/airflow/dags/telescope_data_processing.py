...
def discover_new_fits_files(**context) -> list:
    ...
    try:
        response = s3_client.list_objects_v2(
            Bucket=S3_BUCKET_RAW, Prefix="fits/", StartAfter=f"fits/{since_time.strftime('%Y/%m/%d')}"
        )
...
def validate_fits_files(**context) -> dict:
    ...
    result = {"valid_files": valid_files, "invalid_files": invalid_files}
...
def submit_processing_jobs(**context) -> list:
    ...
    for file_info in valid_files:
        try:
            ...
            response = requests.post(f"{IMAGE_PROCESSOR_URL}/api/v1/processing/jobs/s3", json=job_request, timeout=30)

            if response.status_code == 202:
                job_data = response.json()
                submitted_jobs.append({"job_id": job_data["jobId"], "file_info": file_info, "status": "SUBMITTED"})
...
def monitor_processing_jobs(**context) -> dict:
    ...
    while time.time() - start_time < max_wait_time:
        ...
        for job in submitted_jobs:
            ...
            try:
                response = requests.get(f"{IMAGE_PROCESSOR_URL}/api/v1/processing/jobs/{job_id}", timeout=10)
    ...
    result = {"completed": completed_jobs, "failed": failed_jobs, "running": running_jobs}

    logging.info(
        f"Job monitoring complete: {len(completed_jobs)} completed, "
...
def update_catalog(**context) -> dict:
    ...
    for job in completed_jobs:
        try:
            ...
            response = requests.get(f"{IMAGE_PROCESSOR_URL}/api/v1/processing/jobs/{job_id}/results", timeout=30)
            ...
            if response.status_code == 200:
                ...
                if "detectedObjects" in processing_results:
                    ...
                    catalog_response = requests.post(
                        f"{CATALOG_SERVICE_URL}/api/v1/catalog/objects/batch", json={"objects": objects}, timeout=60
                    )
                    ...
                    if catalog_response.status_code == 201:
                        ...
                        logging.error(f"Failed to update catalog for job {job_id}: " f"{catalog_response.status_code}")
    ...
    for job in completed_jobs:
        try:
            if "output_path" in job:
                ...
                s3_client.copy_object(CopySource=copy_source, Bucket=S3_BUCKET_ARCHIVE, Key=archive_key)
...
for task in [discover_task, validate_task, processing_group, update_catalog_task, archive_task, quality_check_task]:
