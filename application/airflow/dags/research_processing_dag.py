        ...
        "processing_steps": [
            {
                "step": "bias-subtract",
                "algorithm": "default",
                "parameters": {"overscanCorrection": True, "fitMethod": "median"},
            },
            {
                "step": "dark-subtract",
                "algorithm": "scaled-dark",
                "parameters": {"autoScale": True, "temperatureCorrection": False},
            },
            {
                "step": "flat-correct",
                "algorithm": "illumination-corrected",
                "parameters": {"illuminationModel": "polynomial", "polynomialDegree": 3, "maskStars": True},
            },
            {
                "step": "cosmic-ray-remove",
                "algorithm": "lacosmic-v2",
                "parameters": {"sigclip": 4.5, "starPreservation": True, "niter": 4},
            },
        ],
...
def create_processing_request(
    step_config: Dict, session_id: str, current_image_path: str, calibration_frames: Dict, output_config: Dict
) -> Dict[str, Any]:
...
def execute_processing_step(step_config: Dict, **context) -> str:
    ...
    try:
        ...
        if result.get("processingMetrics"):
            context["ti"].xcom_push(key=f"{step_type}_metrics", value=result["processingMetrics"])
...
def create_algorithm_check_task(step_config: Dict) -> PythonOperator:
    ...
    return PythonOperator(
        task_id=f'check_algorithm_{step_config["step"].replace("-", "_")}', python_callable=check_algorithm, dag=dag
    )
