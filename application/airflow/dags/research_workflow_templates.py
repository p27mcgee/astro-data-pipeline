...
def create_algorithm_comparison_dag() -> DAG:
    ...
    discover_algorithms = AlgorithmDiscoveryOperator(
        task_id="discover_available_algorithms", algorithm_type="{{ params.processing_step }}", dag=dag
    )
    ...
    def compare_algorithm_results(**context):
        ...
        for i, task in enumerate(comparison_tasks):
            ...
            results[task_id] = {"output_path": output_path, "metrics": metrics}

        logger.info(f"Algorithm comparison results: {results}")
        return results

    compare_results = PythonOperator(
        task_id="compare_algorithm_results", python_callable=compare_algorithm_results, dag=dag
    )
        ...
        for sigclip in param_grid["sigclip"]:
            for niter in param_grid["niter"]:
                for star_pres in param_grid["starPreservation"]:
                    combinations.append({"sigclip": sigclip, "niter": niter, "starPreservation": star_pres})

        logger.info(f"Generated {len(combinations)} parameter combinations")
        return combinations

    generate_params = PythonOperator(
        task_id="generate_parameter_combinations", python_callable=generate_parameter_combinations, dag=dag
    )
    ...
    def analyze_optimization_results(**context):
        ...
        for task in param_test_tasks:
            ...
            if metrics:
                ...
                if score > best_score:
                    best_score = score
                    best_result = {"task_id": task_id, "metrics": metrics, "score": score}

        logger.info(f"Best parameter combination: {best_result}")
        return best_result

    analyze_results = PythonOperator(
        task_id="analyze_optimization_results", python_callable=analyze_optimization_results, dag=dag
    )
...
def create_custom_research_dag() -> DAG:
    ...
    dag = DAG(
        ...
        params={
            ...
            "analysis_configuration": {"enable_photometry": True, "enable_astrometry": False, "output_catalog": True},
        },
    )
    ...
    validate_workflow = PythonOperator(
        task_id="validate_custom_workflow", python_callable=validate_custom_workflow, dag=dag
    )
    ...
    generate_report = PythonOperator(
        task_id="generate_research_report", python_callable=generate_research_report, dag=dag
    )
