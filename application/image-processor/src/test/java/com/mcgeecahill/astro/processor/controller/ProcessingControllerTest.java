package com.mcgeecahill.astro.processor.controller;
...
import com.mcgeecahill.astro.processor.dto.ProcessingMetricsResponse;
...
/**
 * Unit tests for ProcessingController Focuses on controller logic, validation, and response
 * handling
 */
...
class ProcessingControllerTest {
    ...
    @InjectMocks private ProcessingController processingController;
    ...
    @Test
    void submitJobValidRequestShouldReturn202() throws Exception {
        // Given
    ...
    @Test
    void submitJobServiceExceptionShouldReturn400() throws Exception {
        // Given
        JobSubmissionRequest request = createValidJobRequest();
    ...
    @Test
    void submitJobInvalidJsonShouldReturn400() throws Exception {
        // When & Then
        mockMvc.perform(
                        post("/api/v1/processing/jobs")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void getJobStatusExistingJobShouldReturnJob() throws Exception {
        // Given
        String jobId = "job_123";
        JobStatusResponse expectedResponse = createJobStatusResponse(jobId);

        when(processingJobService.getJobStatus(jobId)).thenReturn(Optional.of(expectedResponse));
    ...
    @Test
    void getJobStatusNonExistentJobShouldReturn404() throws Exception {
        // Given
        String jobId = "nonexistent_job";

        when(processingJobService.getJobStatus(jobId)).thenReturn(Optional.empty());
    ...
    @Test
    void listJobsWithoutFiltersShouldReturnAllJobs() throws Exception {
        // Given
        List<JobStatusResponse> jobs =
                Arrays.asList(createJobStatusResponse("job_1"), createJobStatusResponse("job_2"));
    ...
    @Test
    void listJobsWithFiltersShouldReturnFilteredJobs() throws Exception {
        // Given
        List<JobStatusResponse> jobs = Collections.singletonList(createJobStatusResponse("job_1"));
        Page<JobStatusResponse> jobPage = new PageImpl<>(jobs);

        when(processingJobService.listJobs(
                        eq("user123"),
                        eq(ProcessingJob.ProcessingStatus.RUNNING),
                        eq(ProcessingJob.ProcessingType.FULL_CALIBRATION),
                        eq(0),
                        eq(10)))
                .thenReturn(jobPage);
    ...
    @Test
    void listJobsWithPaginationShouldReturnPagedResults() throws Exception {
        // Given
        List<JobStatusResponse> jobs = Collections.singletonList(createJobStatusResponse("job_1"));
        Page<JobStatusResponse> jobPage = new PageImpl<>(jobs);
        ...
        // When & Then
        mockMvc.perform(get("/api/v1/processing/jobs").param("page", "2").param("size", "5"))
                .andExpect(status().isOk());

        verify(processingJobService, times(1)).listJobs(isNull(), isNull(), isNull(), eq(2), eq(5));
    }

    // ========== Job Cancellation Tests ==========

    @Test
    void cancelJobValidJobShouldReturnCancelledJob() throws Exception {
        // Given
        String jobId = "job_123";
        JobStatusResponse cancelledJob =
                createJobStatusResponse(jobId, ProcessingJob.ProcessingStatus.CANCELLED);

        when(processingJobService.cancelJob(jobId)).thenReturn(cancelledJob);
    ...
    @Test
    void cancelJobNonExistentJobShouldReturn400() throws Exception {
        // Given
        String jobId = "nonexistent_job";
    ...
    @Test
    void retryJobFailedJobShouldReturnQueuedJob() throws Exception {
        // Given
        String jobId = "failed_job";
        JobStatusResponse retriedJob =
                createJobStatusResponse(jobId, ProcessingJob.ProcessingStatus.QUEUED);

        when(processingJobService.retryJob(jobId)).thenReturn(retriedJob);
    ...
    @Test
    void retryJobNonFailedJobShouldReturn400() throws Exception {
        // Given
    ...
    @Test
    void getMetricsShouldReturnProcessingMetrics() throws Exception {
        // Given
        ProcessingMetricsResponse metrics =
                ProcessingMetricsResponse.createBasic(100L, 85L, 10L, 5000.0, 5, 3);

        when(processingJobService.getProcessingMetrics()).thenReturn(metrics);
    ...
    @Test
    void submitBatchJobsValidRequestsShouldCreateMultipleJobs() throws Exception {
        // Given
        ...
        when(processingJobService.submitBatchJobs(any())).thenReturn(expectedJobIds);

        // When & Then
        mockMvc.perform(
                        post("/api/v1/processing/jobs/batch")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void submitBatchJobsServiceExceptionShouldReturn400() throws Exception {
        // Given
        Map<String, JobSubmissionRequest> batchRequest = new HashMap<>();
        batchRequest.put("job1", createValidJobRequest());
    ...
    @Test
    void cleanupJobsShouldDeleteOldJobs() throws Exception {
        // Given
        Map<String, Object> cleanupResult = new HashMap<>();
        cleanupResult.put("deletedCount", 5);
        cleanupResult.put("status", "ALL");

        when(processingJobService.cleanupJobs(eq(7), isNull())).thenReturn(cleanupResult);

        // When & Then
        mockMvc.perform(delete("/api/v1/processing/jobs").param("olderThanDays", "7"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    void cleanupJobsWithStatusFilterShouldDeleteFilteredJobs() throws Exception {
        // Given
        Map<String, Object> cleanupResult = new HashMap<>();
    ...
    @Test
    void getHealthStatusShouldReturnSystemHealth() throws Exception {
        // Given
        ...
        when(processingJobService.getHealthStatus()).thenReturn(healthStatus);
    ...
    private JobStatusResponse createJobStatusResponse(
            String jobId, ProcessingJob.ProcessingStatus status) {
