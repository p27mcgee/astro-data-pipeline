...
import com.mcgeecahill.astro.processor.repository.ProcessingJobRepository;
...
@TestPropertySource(properties = {"spring.profiles.active=${SPRING_PROFILES_ACTIVE:test}"})
...
class ProcessingControllerIT {
    ...
    @Test
    void submitJobValidRequestShouldCreateJobAndReturn202() throws Exception {
        // Given
    ...
    @Test
    void submitJobInvalidRequestShouldReturn400() throws Exception {
    ...
    @Test
    void submitJobWithNullPriorityShouldUseDefaultPriority() throws Exception {
        // Given
        JobSubmissionRequest request =
                JobSubmissionRequest.builder()
                        .inputBucket("test-input-bucket")
    ...
    @Test
    void getJobStatusNonExistingJobShouldReturn404() throws Exception {
        // When & Then
    ...
    @Test
    void listJobsWithStatusFilterShouldReturnFilteredJobs() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/v1/processing/jobs").param("status", "QUEUED"))
                .andExpect(status().isOk())
    ...
    void listJobsWithPaginationShouldReturnPagedResults() throws Exception {
        ...
        for (int i = 1; i <= 25; i++) {
            jobRepository.save(
                    createSampleJob(
                            "job_" + String.format("%03d", i),
                            ProcessingJob.ProcessingStatus.QUEUED));
        }

        // When & Then - First page
        mockMvc.perform(get("/api/v1/processing/jobs").param("page", "0").param("size", "10"))
                .andExpect(status().isOk())
        ...
        mockMvc.perform(get("/api/v1/processing/jobs").param("page", "1").param("size", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(10))
                .andExpect(jsonPath("$.number").value(1));

        // When & Then - Last page
        mockMvc.perform(get("/api/v1/processing/jobs").param("page", "2").param("size", "10"))
                .andExpect(status().isOk())
    ...
    @Test
    void cancelJobExistingQueuedJobShouldCancelSuccessfully() throws Exception {
        // Given
    ...
    @Test
    void cancelJobCompletedJobShouldReturn400() throws Exception {
        // Given
        ProcessingJob completedJob =
                createSampleJob("completed_job", ProcessingJob.ProcessingStatus.COMPLETED);
    ...
    @Test
    void cancelJobNonExistingJobShouldReturn400() throws Exception {
        // When & Then
    ...
    @Test
    void retryJobFailedJobShouldRetrySuccessfully() throws Exception {
        // Given
        ProcessingJob failedJob =
                createSampleJob("failed_job", ProcessingJob.ProcessingStatus.FAILED);
    ...
    @Test
    void retryJobJobExceededMaxRetriesShouldReturn400() throws Exception {
        // Given
        ProcessingJob failedJob =
                createSampleJob("max_retries_job", ProcessingJob.ProcessingStatus.FAILED);
    ...
    void retryJobNonFailedJobShouldReturn400() throws Exception {
        // Given
        ProcessingJob runningJob =
                createSampleJob("running_job", ProcessingJob.ProcessingStatus.RUNNING);
    ...
    void getMetricsShouldReturnProcessingMetrics() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/processing/metrics"))
                ...
                .andExpect(
                        jsonPath("$.averageProcessingTimeMs")
                                .value(90000.0)) // (60000 + 120000) / 2
                // TODO .andExpect(jsonPath("$.systemStatus").value("HEALTHY"));
                .andExpect(
                        jsonPath("$.systemStatus")
                                .value("CRITICAL")); // 40% success rate = critical
    }
    ...
    @Test
    void apiResponseHeadersShouldIncludeCorrectHeaders() throws Exception {
        // Given
        JobSubmissionRequest request =
                JobSubmissionRequest.builder()
    ...
    @Test
    void apiErrorHandlingInvalidJsonShouldReturn400() throws Exception {
        // When & Then
        mockMvc.perform(
                        post("/api/v1/processing/jobs")
                                .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    void apiContentNegotiationUnsupportedMediaTypeShouldReturn415() throws Exception {
        // When & Then
        mockMvc.perform(
                        post("/api/v1/processing/jobs")
