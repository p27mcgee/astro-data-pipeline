package com.mcgeecahill.astro.processor.entity;
...
public final class ProcessingJob {
    ...
    @CollectionTable(name = "processing_job_metadata", joinColumns = @JoinColumn(name = "job_id"))
    ...
    @ElementCollection(fetch = FetchType.LAZY)
    @CollectionTable(name = "processing_job_steps", joinColumns = @JoinColumn(name = "job_id"))
    @Column(name = "step_name")
    @Enumerated(EnumType.STRING)
    private List<ProcessingStep> completedSteps;

    // Performance metrics
    @Embedded private ProcessingMetrics metrics;
    ...
    public boolean canRetry() {
        return retryCount < maxRetries
                && (status == ProcessingStatus.FAILED || status == ProcessingStatus.RETRY);
    }
    ...
    public void markAsCompleted() {
        ...
        if (this.startedAt != null) {
            this.processingDurationMs =
                    java.time.Duration.between(this.startedAt, this.completedAt).toMillis();
        }
    }
