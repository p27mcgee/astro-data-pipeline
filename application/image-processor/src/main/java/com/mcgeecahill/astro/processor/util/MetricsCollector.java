...
public class MetricsCollector {
    ...
    public MetricsCollector(MeterRegistry meterRegistry) {
        ...
        this.processingSuccessCounter =
                Counter.builder("fits_processing_success_total")
                        .description("Total number of successful FITS processing operations")
                        .register(meterRegistry);

        this.processingFailureCounter =
                Counter.builder("fits_processing_errors_total")
                        .description("Total number of failed FITS processing operations")
                        .register(meterRegistry);

        this.cosmicRayDetectionCounter =
                Counter.builder("cosmic_ray_detection_total")
                        .description("Total number of cosmic rays detected and removed")
                        .register(meterRegistry);

        this.darkSubtractionCounter =
                Counter.builder("dark_subtraction_operations_total")
                        .description("Total number of dark subtraction operations")
                        .register(meterRegistry);

        this.flatCorrectionCounter =
                Counter.builder("flat_correction_operations_total")
                        .description("Total number of flat correction operations")
                        .register(meterRegistry);

        // Initialize timers
        this.processingTimer =
                Timer.builder("fits_processing_duration_seconds")
                        .description("Time taken to process FITS images")
                        .register(meterRegistry);

        this.ioTimer =
                Timer.builder("fits_io_duration_seconds")
    ...
    public void recordImageQuality(double qualityScore) {
        ...
        if (qualityScore >= MetricsConstants.EXCELLENT_QUALITY_THRESHOLD) {
            qualityCategory = "excellent";
        } else if (qualityScore >= MetricsConstants.GOOD_QUALITY_THRESHOLD) {
            qualityCategory = "good";
        } else if (qualityScore >= MetricsConstants.FAIR_QUALITY_THRESHOLD) {
    ...
    public void recordCosmicRayDetection(int cosmicRaysDetected) {
        ...
        sample.stop(
                Timer.builder("cosmic_ray_count_distribution")
    ...
    public void recordFileSize(String fileType, long sizeBytes) {
        ...
        long oneMB =
                ImageProcessingConstants.BYTES_PER_KILOBYTE
                        * ImageProcessingConstants.BYTES_PER_KILOBYTE;
        long oneHundredMB = 100 * oneMB;

        if (sizeBytes < oneMB) { // < 1MB
            Counter.builder("fits_file_size_category_total")
                    .tag("category", "small")
                    .register(meterRegistry)
                    .increment();
        } else if (sizeBytes < oneHundredMB) { // < 100MB
            Counter.builder("fits_file_size_category_total")
                    .tag("category", "medium")
                    .register(meterRegistry)
                    .increment();
