package com.mcgeecahill.astro.processor.repository;
...
public interface ProcessingJobRepository extends JpaRepository<ProcessingJob, Long> {
    ...
    Page<ProcessingJob> findByUserIdAndStatus(
            String userId, ProcessingJob.ProcessingStatus status, Pageable pageable);
    ...
    List<ProcessingJob> findByStatusAndPriorityOrderByCreatedAt(
            ProcessingJob.ProcessingStatus status, Integer priority);
    ...
    @Query(
            "SELECT j FROM ProcessingJob j WHERE j.status = 'COMPLETED' AND j.completedAt <"
                    + " :cutoffDate")
    List<ProcessingJob> findCompletedJobsOlderThan(@Param("cutoffDate") LocalDateTime cutoffDate);

    /** Find jobs by processing type */
    Page<ProcessingJob> findByProcessingType(
            ProcessingJob.ProcessingType processingType, Pageable pageable);

    /** Find jobs by input bucket and object key */
    Optional<ProcessingJob> findByInputBucketAndInputObjectKey(
            String inputBucket, String inputObjectKey);
    ...
    @Query(
            "SELECT j FROM ProcessingJob j WHERE j.status = :status ORDER BY j.priority ASC,"
                    + " j.createdAt ASC")
    List<ProcessingJob> findByStatusOrderByPriorityAndCreatedAt(
            @Param("status") ProcessingJob.ProcessingStatus status);

    /** Find jobs that have exceeded max retries */
    @Query(
            "SELECT j FROM ProcessingJob j WHERE j.retryCount >= j.maxRetries AND j.status ="
                    + " 'FAILED'")
    List<ProcessingJob> findJobsExceededMaxRetries();

    /** Find long running jobs */
    @Query(
            "SELECT j FROM ProcessingJob j WHERE j.status = 'RUNNING' AND j.startedAt <"
                    + " :timeThreshold")
    ...
    @Query(
            "SELECT AVG(j.processingDurationMs) FROM ProcessingJob j WHERE j.status = 'COMPLETED'"
                    + " AND j.processingDurationMs IS NOT NULL")
    Double getAverageProcessingTime();

    /** Get processing statistics by type */
    @Query(
            "SELECT j.processingType, COUNT(j), AVG(j.processingDurationMs) "
    ...
    /** Find jobs for retry */
    @Query(
            "SELECT j FROM ProcessingJob j WHERE j.status = 'FAILED' AND j.retryCount <"
                    + " j.maxRetries")
