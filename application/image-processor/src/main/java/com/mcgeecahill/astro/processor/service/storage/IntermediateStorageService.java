...
/**
 * Service for managing intermediate storage during multi-step processing workflows. Uses the
 * configured StorageProvider (S3, MinIO, or filesystem).
 */
@Slf4j
@Service
...
public class IntermediateStorageService {
    ...
    public String storeIntermediateResult(
            String sessionId,
            String stepType,
        ...
        try {
            ...
            String key =
                    buildIntermediateKey(sessionId, stepType, originalImagePath, customOutputPath);

            log.info(
                    "Storing intermediate result: bucket={}, key={}, size={} bytes",
                    bucket,
                    key,
                    processedData.length);

            storageProvider.storeData(bucket, key, processedData);
            ...
            log.error(
                    "Failed to store intermediate result for session: {}, step: {}",
                    sessionId,
                    stepType,
                    e);
    ...
    public String moveFinalResult(
            String intermediateResultPath, String finalBucket, String finalPath) {
        try {
            log.info(
                    "Moving final result from {} to bucket: {}, path: {}",
            ...
            byte[] data = storageProvider.retrieveData(sourceBucket, sourceKey);
            ...
            storageProvider.storeData(targetBucket, finalKey, data);
    ...
    public List<IntermediateFileInfo> listIntermediateResults(String sessionId) {
        try {
            ...
            List<String> s3Objects = storageProvider.listObjects(defaultIntermediateBucket, prefix);

            List<IntermediateFileInfo> results =
                    s3Objects.stream()
    ...
    public void cleanupSessionFiles(String sessionId, boolean keepFinalResult) {
        try {
            log.info(
                    "Cleaning up session files for: {}, keepFinal: {}", sessionId, keepFinalResult);
            ...
            for (IntermediateFileInfo fileInfo : sessionFiles) {
                ...
                try {
                    storageProvider.deleteObject(defaultIntermediateBucket, fileInfo.getKey());
    ...
    private String buildIntermediateKey(
            String sessionId, String stepType, String originalImagePath, String customOutputPath) {
        ...
        return String.format("sessions/%s/%s/%s/%s", sessionId, stepType, timestamp, filename);
    }
    ...
    private IntermediateFileInfo parseIntermediateFileInfo(String key, String sessionId) {
        try {
            ...
            if (keyParts.length < 4
                    || !keyParts[0].equals("sessions")
                    || !keyParts[1].equals(sessionId)) {
                return null;
            }
