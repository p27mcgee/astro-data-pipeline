 ...
 *
 * <p>Manages processing contexts, tracks experiments, and provides processing ID generation and
...
public class ProcessingContextService {
    ...
    public ProcessingContext createProductionContext(
            String sessionId,
        ...
        ProcessingContext context =
                ProcessingContext.builder()
                        ...
                        .productionContext(
                                ProcessingContext.ProductionContext.builder()
                                        ...
                                        .build())
                        .dataLineage(
                                ProcessingContext.DataLineage.builder().processingDepth(0).build())
                        .build();
        ...
        log.info(
                "Created production processing context: {} for session: {}",
                context.getProcessingId(),
                sessionId);
    ...
    public ProcessingContext createExperimentalContext(
            String sessionId,
        ...
        ProcessingContext context =
                ProcessingContext.builder()
                        ...
                        .experimentContext(
                                ProcessingContext.ExperimentContext.builder()
        ...
        log.info(
                "Created experimental processing context: {} for experiment: {} by researcher: {}",
                context.getProcessingId(),
                experimentName,
                researcherId);
    ...
    public ProcessingContext createDerivedContext(String parentProcessingId, String newSessionId) {
        ProcessingContext parentContext =
                getProcessingContext(parentProcessingId)
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Parent processing context not found: "
                                                        + parentProcessingId));
        ...
        log.info(
                "Created derived processing context: {} from parent: {}",
                derivedContext.getProcessingId(),
                parentProcessingId);
    ...
    public void updateDataLineage(
            ...
            String inputImageChecksum,
            Map<String, String> calibrationFrames) {
        ProcessingContext context =
                getProcessingContext(processingId)
                        .orElseThrow(
                                () ->
                                        new RuntimeException(
                                                "Processing context not found: " + processingId));
    ...
    public Optional<ProcessingContext.ExperimentContext> getExperimentContext(String processingId) {
        return getProcessingContext(processingId).map(ProcessingContext::getExperimentContext);
    }
    ...
    public Optional<ProcessingContext.ProductionContext> getProductionContext(String processingId) {
        return getProcessingContext(processingId).map(ProcessingContext::getProductionContext);
    }
                ...
                .filter(
                        entry -> {
                            ProcessingContext.ExperimentContext expContext =
                                    entry.getValue().getExperimentContext();
                            return expContext != null
                                    && researcherId.equals(expContext.getResearcherId());
                        })
    ...
    public ProcessingContext createContextWithActiveWorkflow(
            String workflowName,
            String sessionId,
            ProcessingContext.ProcessingType processingType) {
        // Get active workflow version for this workflow type
        Optional<WorkflowVersion> activeWorkflow =
                workflowVersionService.getActiveWorkflowForProcessing(
                        workflowName, processingType, sessionId);

        if (activeWorkflow.isEmpty()) {
            log.warn(
                    "No active workflow found for {} {}, using default",
                    workflowName,
                    processingType);
    ...
    public ProcessingContext createContextFromWorkflowVersion(
            WorkflowVersion workflowVersion, String sessionId) {
        String processingId =
                ProcessingContext.generateProcessingId(
        ...
        ProcessingContext.ProcessingContextBuilder contextBuilder =
                ProcessingContext.builder()
        ...
        if (workflowVersion.getProcessingType() == ProcessingContext.ProcessingType.EXPERIMENTAL) {
            contextBuilder.experimentContext(
                    ProcessingContext.ExperimentContext.builder()
                            ...
                            .build());
        } else if (workflowVersion.getProcessingType()
                == ProcessingContext.ProcessingType.PRODUCTION) {
            contextBuilder.productionContext(
                    ProcessingContext.ProductionContext.builder()
                            .priority(1)
                            .dataReleaseVersion("DR1")
                            .calibrationFrameVersions(new HashMap<>())
        ...
        workflowVersionService.updateWorkflowUsage(
                workflowVersion.getWorkflowName(),
                workflowVersion.getWorkflowVersion(),
                workflowVersion.getProcessingType());

        log.info(
                "Created processing context from workflow {} {}: {} for session: {}",
                workflowVersion.getWorkflowName(),
                workflowVersion.getWorkflowVersion(),
                context.getProcessingId(),
                sessionId);
    ...
    public ProcessingContext getOrCreateWorkflowContext(
            String workflowName,
            String sessionId,
            ProcessingContext.ProcessingType processingType,
        ...
        if (workflowVersion != null) {
            Optional<WorkflowVersion> specificWorkflow =
                    workflowVersionService.getWorkflowVersion(
    ...
    private ProcessingContext createDefaultContext(
            String sessionId, ProcessingContext.ProcessingType processingType) {
        ProcessingContext.ProcessingContextBuilder contextBuilder =
                ProcessingContext.builder()
    ...
    public Optional<WorkflowVersion> getRecommendedWorkflow(
            String workflowName, ProcessingContext.ProcessingType processingType) {
        return workflowVersionService.getActiveWorkflowForProcessing(
                workflowName, processingType, "default");
    }
}
