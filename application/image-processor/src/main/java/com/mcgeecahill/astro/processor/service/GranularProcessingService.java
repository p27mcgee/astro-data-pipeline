...
import com.mcgeecahill.astro.processor.service.algorithm.AlgorithmRegistryService;
...
public class GranularProcessingService {
    ...
    public GranularProcessingResponse applyDarkSubtraction(GranularProcessingRequest request) {
        log.info(
                "Starting dark subtraction for session: {}, image: {}",
        ...
        try {
            ...
            byte[] processedData =
                    fitsProcessingService.applyDarkSubtractionGranular(
    ...
    public GranularProcessingResponse applyFlatFieldCorrection(GranularProcessingRequest request) {
        log.info(
                "Starting flat field correction for session: {}, image: {}",
                request.getSessionId(),
                request.getImagePath());
        ...
        try {
            ...
            byte[] processedData =
                    fitsProcessingService.applyFlatFieldCorrectionGranular(
    ...
    public GranularProcessingResponse removeCosmicRays(GranularProcessingRequest request) {
        log.info(
                "Starting cosmic ray removal for session: {}, image: {}",
                request.getSessionId(),
                request.getImagePath());
        ...
        try {
            ...
            byte[] processedData =
                    fitsProcessingService.removeCosmicRaysGranular(
    ...
    public GranularProcessingResponse applyBiasSubtraction(GranularProcessingRequest request) {
        log.info(
                "Starting bias subtraction for session: {}, image: {}",
                request.getSessionId(),
                request.getImagePath());
        ...
        try {
            ...
            byte[] processedData =
                    fitsProcessingService.applyBiasSubtractionGranular(
    ...
    public CustomWorkflowResponse executeCustomWorkflow(CustomWorkflowRequest request) {
        log.info(
                "Starting custom workflow for session: {}, {} steps",
                request.getSessionId(),
        ...
        try {
            for (int i = 0; i < request.getSteps().size(); i++) {
                ...
                try {
                    ...
                    GranularProcessingRequest stepRequest =
                            GranularProcessingRequest.builder()
                    ...
                    // Execute the step
                    GranularProcessingResponse stepResponse =
                            executeWorkflowStep(step.getStepType(), stepRequest);
                    ...
                    if ("SUCCESS".equals(stepResponse.getStatus())) {
                        ...
                        stepResults.add(
                                CustomWorkflowResponse.StepResult.builder()
                        ...
                        if (step.getOptional()) {
                            log.warn(
                                    "Optional step {} failed, continuing workflow",
                                    step.getStepType());
                            stepResults.add(
                                    CustomWorkflowResponse.StepResult.builder()
                                            .stepType(step.getStepType())
                                            ...
                                            .errorMessage(
                                                    "Optional step failed: "
                                                            + stepResponse.getErrorMessage())
                                            .build());
                        } else {
                            // Required step failed, abort workflow
                            throw new RuntimeException(
                                    "Required step failed: " + stepResponse.getErrorMessage());
                        }
                    }
            ...
            long totalProcessingTime =
                    java.time.Duration.between(workflowStartTime, workflowEndTime).toMillis();
            ...
            return CustomWorkflowResponse.builder()
                    ...
                    .intermediateFiles(
                            request.getCleanupIntermediates()
                                    ? Collections.emptyList()
                                    : intermediateFiles)
                    .workflowMetrics(
                            buildWorkflowMetrics(request, stepResults, totalProcessingTime))
                    .build();
    ...
    private GranularProcessingResponse executeWorkflowStep(
            String stepType, GranularProcessingRequest request) {
    ...
    private Map<String, Object> buildProcessingMetrics(
            GranularProcessingRequest request, long processingTime) {
    ...
    private Map<String, Object> buildWorkflowMetrics(
            CustomWorkflowRequest request,
            List<CustomWorkflowResponse.StepResult> stepResults,
            long totalTime) {
        Map<String, Object> metrics = new HashMap<>();
        metrics.put("totalSteps", request.getSteps().size());
        metrics.put(
                "successfulSteps",
                stepResults.stream().mapToLong(r -> "SUCCESS".equals(r.getStatus()) ? 1 : 0).sum());
        metrics.put(
                "failedSteps",
                stepResults.stream().mapToLong(r -> "FAILED".equals(r.getStatus()) ? 1 : 0).sum());
        metrics.put(
                "skippedSteps",
                stepResults.stream().mapToLong(r -> "SKIPPED".equals(r.getStatus()) ? 1 : 0).sum());
        metrics.put("totalProcessingTimeMs", totalTime);
        metrics.put(
                "averageStepTimeMs", stepResults.isEmpty() ? 0 : totalTime / stepResults.size());
        return metrics;
    }
    ...
    private ProcessingContext getOrCreateProcessingContext(GranularProcessingRequest request) {
        ...
        if (request.getProcessingId() != null) {
            return processingContextService
                    .getProcessingContext(request.getProcessingId())
                    .orElseThrow(
                            () ->
                                    new RuntimeException(
                                            "Processing context not found: "
                                                    + request.getProcessingId()));
        }

        // Check if context exists for this session
        Optional<ProcessingContext> existingContext =
                processingContextService.getProcessingContextBySession(request.getSessionId());
        ...
        String processingType =
                request.getProcessingType() != null
                        ? request.getProcessingType().toLowerCase()
                        : "production";
    ...
    private GranularProcessingResponse buildEnhancedResponse(
            GranularProcessingRequest request,
        ...
        GranularProcessingResponse.GranularProcessingResponseBuilder responseBuilder =
                GranularProcessingResponse.builder()
                        .status("SUCCESS")
                        .outputPath(outputPath)
                        .sessionId(request.getSessionId())
                        .stepId(stepId)
        ...
        if (processingContext.isExperimental()
                && processingContext.getExperimentContext() != null) {
            ProcessingContext.ExperimentContext expContext =
                    processingContext.getExperimentContext();
            responseBuilder.experimentInfo(
                    GranularProcessingResponse.ExperimentInfo.builder()
        ...
        if (processingContext.isProduction() && processingContext.getProductionContext() != null) {
            ProcessingContext.ProductionContext prodContext =
                    processingContext.getProductionContext();
            responseBuilder.productionInfo(
                    GranularProcessingResponse.ProductionInfo.builder()
        ...
        if (processingContext.getDataLineage() != null) {
            ...
            responseBuilder.dataLineage(
                    GranularProcessingResponse.DataLineageInfo.builder()
    ...
    private void updateProcessingLineage(
            ProcessingContext processingContext, GranularProcessingRequest request) {
    ...
    private String calculateImageChecksum(String imagePath) {
        try {
            ...
            return "md5:"
                    + Integer.toHexString((imagePath + System.currentTimeMillis()).hashCode());
        } catch (Exception e) {
