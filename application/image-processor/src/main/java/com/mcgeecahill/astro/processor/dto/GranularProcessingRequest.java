...
public class GranularProcessingRequest {
    ...
    @Schema(
            description = "S3 path to the input FITS image",
            example = "raw-data/session123/image001.fits")
    @JsonProperty("imagePath")
    private String imagePath;

    @Schema(
            description = "S3 path to calibration frame (dark, flat, bias)",
            example = "raw-data/calibration/master_dark.fits")
    @JsonProperty("calibrationPath")
    private String calibrationPath;

    @NotBlank(message = "Session ID is required")
    @Schema(
            description = "Processing session identifier for grouping intermediate results",
            example = "session-20240101-123456")
    @JsonProperty("sessionId")
    private String sessionId;

    @Schema(
            description = "Algorithm implementation to use",
            example = "lacosmic-v2",
    ...
    @Schema(
            description = "Output bucket for intermediate results",
            example = "intermediate-data",
            defaultValue = "intermediate-data")
    ...
    @Schema(
            description =
                    "Processing context ID for tracking production vs experimental processing",
            example = "exp-20240928-a1b2c3d4-5e6f-7890-abcd-ef1234567890")
    @JsonProperty("processingId")
    private String processingId;

    @Schema(
            description = "Processing type: production, experimental, test",
