...
import com.mcgeecahill.astro.processor.constants.JobManagementConstants;
...
public class ProcessingJobService {
    ...
    public JobStatusResponse submitJob(JobSubmissionRequest request) {
        log.info(
                "Submitting new processing job for input: {}/{}",
                request.getInputBucket(),
                request.getInputObjectKey());
                ...
                ProcessingJob.builder()
                        ...
                        .priority(
                                request.getPriority() != null
                                        ? request.getPriority()
                                        : JobManagementConstants.DEFAULT_PRIORITY)
                        ...
                        .maxRetries(
                                request.getMaxRetries() != null
                                        ...
                                        : JobManagementConstants.DEFAULT_MAX_RETRIES)
                        .retryCount(0)
                        .build();
        ...
        metricsCollector.updateQueueSize(
                jobRepository.countByStatus(ProcessingJob.ProcessingStatus.QUEUED));
    ...
    public Optional<JobStatusResponse> getJobStatus(String jobId) {
        return jobRepository.findByJobId(jobId).map(JobStatusResponse::fromEntity);
    }

    /** List jobs with pagination and filtering */
    @Transactional(readOnly = true)
    public Page<JobStatusResponse> listJobs(
            String userId,
        ...
        if (userId != null && status != null && processingType != null) {
            jobs =
                    jobRepository.findByUserIdAndStatusAndProcessingType(
                    ...
                    try {
                        ...
                        metricsCollector.updateActiveJobs(
                                jobRepository.countByStatus(
                                        ProcessingJob.ProcessingStatus.RUNNING));
                        ...
                        jobRepository.save(job);
                        metricsCollector.recordProcessingSuccess(
                                job, job.getProcessingDurationMs());
    ...
    public ProcessingMetricsResponse getProcessingMetrics() {
        ...
        Double avgProcessingTime =
                jobRepository.findCompletedJobs().stream()
    ...
    private Optional<ProcessingJob> findNextJobToProcess() {
        ...
        List<ProcessingJob> highPriorityJobs =
                jobRepository.findByStatusAndPriorityOrderByCreatedAt(
                        ProcessingJob.ProcessingStatus.QUEUED,
                        JobManagementConstants.HIGH_PRIORITY);
        ...
        List<ProcessingJob> normalPriorityJobs =
                jobRepository.findByStatusAndPriorityOrderByCreatedAt(
                        ProcessingJob.ProcessingStatus.QUEUED,
                        JobManagementConstants.DEFAULT_PRIORITY);
        ...
        List<ProcessingJob> lowPriorityJobs =
                jobRepository.findByStatusAndPriorityOrderByCreatedAt(
                        ProcessingJob.ProcessingStatus.QUEUED, JobManagementConstants.LOW_PRIORITY);
    ...
    private void simulateProcessing(ProcessingJob job) throws InterruptedException {
        ...
        for (int i = 0; i < steps.length; i++) {
            ...
            Thread.sleep(
                    JobManagementConstants.SIMULATION_BASE_SLEEP_MS
                            + (long)
                                    (Math.random()
                                            * JobManagementConstants.SIMULATION_RANDOM_SLEEP_MS));
        }
    }
    ...
    private String generateJobId() {
        return "job_"
                + UUID.randomUUID()
                        .toString()
                        .replace("-", "")
                        .substring(0, JobManagementConstants.JOB_ID_LENGTH);
    }
    ...
    @Transactional
    public Map<String, Object> cleanupJobs(
            int olderThanDays, ProcessingJob.ProcessingStatus status) {
        ...
        if (status != null) {
            ...
            jobsToDelete =
                    jobRepository.findCompletedJobsOlderThan(cutoffDate).stream()
        ...
        if (!jobsToDelete.isEmpty()) {
            ...
            log.info(
                    "Cleaned up {} jobs older than {} days with status {}",
                    deletedCount,
                    olderThanDays,
                    status);
        }
    ...
    public Map<String, Object> getHealthStatus() {
        ...
        if (queuedJobs > MetricsConstants.HEALTH_DEGRADED_QUEUE_THRESHOLD
                || failedJobs > MetricsConstants.HEALTH_DEGRADED_FAILED_THRESHOLD) {
            status = "DEGRADED";
        }
        if (queuedJobs > MetricsConstants.HEALTH_CRITICAL_QUEUE_THRESHOLD
                || failedJobs > MetricsConstants.HEALTH_CRITICAL_FAILED_THRESHOLD) {
            status = "CRITICAL";
        }
