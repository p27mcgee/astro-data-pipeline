...
import com.mcgeecahill.astro.processor.service.storage.StorageProvider;
...
/**
 * AWS S3 implementation of StorageProvider. Activated when storage.type=s3 or storage.type=minio
...
@ConditionalOnProperty(
        name = "astro.storage.type",
        havingValue = "s3",
        matchIfMissing = true // Default to S3 for backward compatibility
        )
public final class S3Service implements StorageProvider {
    ...
    public String storeData(String bucketName, String key, byte[] data) {
        try {
            log.debug(
                    "Storing data: bucket={}, key={}, size={} bytes", bucketName, key, data.length);
            ...
            log.error("Failed to store data: bucket={}, key={}", bucketName, key, e);
    ...
    public String storeData(
            String bucketName, String key, InputStream inputStream, long contentLength) {
        try {
            log.debug(
                    "Storing data from stream: bucket={}, key={}, size={} bytes",
                    bucketName,
                    key,
                    contentLength);
            ...
            log.error("Failed to store data from stream: bucket={}, key={}", bucketName, key, e);
            throw new RuntimeException("Failed to store data in S3", e);
        }
    }
    ...
    public byte[] retrieveData(String bucketName, String key) {
        try {
            log.debug("Retrieving data: bucket={}, key={}", bucketName, key);

            InputStream inputStream = s3Template.download(bucketName, key).getInputStream();
            byte[] data = inputStream.readAllBytes();

            log.debug("Successfully retrieved {} bytes", data.length);
            return data;

        } catch (Exception e) {
            log.error("Failed to retrieve data: bucket={}, key={}", bucketName, key, e);
     ...
     */
    @Override
    public InputStream retrieveDataAsStream(String bucketName, String key) {
        try {
            log.debug("Retrieving data as stream: bucket={}, key={}", bucketName, key);
            return s3Template.download(bucketName, key).getInputStream();

        } catch (Exception e) {
            log.error("Failed to retrieve data as stream: bucket={}, key={}", bucketName, key, e);
            throw new RuntimeException("Failed to retrieve data from S3", e);
        }
    }
    ...
    public List<String> listObjects(String bucketName, String prefix) {
        try {
            log.debug("Listing objects: bucket={}, prefix={}", bucketName, prefix);
    ...
    public void deleteObject(String bucketName, String key) {
        try {
            log.debug("Deleting object: bucket={}, key={}", bucketName, key);
    ...
    public String generatePresignedUrl(String bucketName, String key, int expirationMinutes) {
        try {
            log.debug(
                    "Generating presigned URL: bucket={}, key={}, expiration={}min",
                    bucketName,
                    key,
                    expirationMinutes);
