...
import com.mcgeecahill.astro.processor.constants.AsyncExecutorConstants;
...
public class AsyncConfig implements AsyncConfigurer {
    ...
    public Executor imageProcessingExecutor() {
        ...
        executor.setKeepAliveSeconds(AsyncExecutorConstants.PROCESSING_EXECUTOR_KEEP_ALIVE_SECONDS);
        ...
        executor.setAwaitTerminationSeconds(
                AsyncExecutorConstants.PROCESSING_EXECUTOR_TERMINATION_TIMEOUT_SECONDS);

        executor.initialize();

        log.info(
                "Initialized image processing executor with core pool size: {}, max pool size: {},"
                        + " queue capacity: {}",
    ...
    public Executor batchProcessingExecutor() {
        ...
        executor.setKeepAliveSeconds(AsyncExecutorConstants.BATCH_EXECUTOR_KEEP_ALIVE_SECONDS);
        executor.setAllowCoreThreadTimeOut(true);
        ...
        executor.setWaitForTasksToCompleteOnShutdown(true);
        executor.setAwaitTerminationSeconds(
                AsyncExecutorConstants.BATCH_EXECUTOR_TERMINATION_TIMEOUT_SECONDS);

        executor.initialize();

        log.info(
                "Initialized batch processing executor with core pool size: {}, max pool size: {},"
                        + " queue capacity: {}",
                executor.getCorePoolSize(),
                executor.getMaxPoolSize(),
                executor.getQueueCapacity());
    ...
    private static final class CustomRejectedExecutionHandler implements RejectedExecutionHandler {
        @Override
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            if (!executor.isShutdown()) {
                log.warn(
                        "Task rejected by executor: {}. Queue size: {}, Active threads: {}",
